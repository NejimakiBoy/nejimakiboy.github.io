...
async function main() {
  ...
  */
  ...
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(col0), gl.STATIC_DRAW);


  gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer1);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(col1), gl.STATIC_DRAW);


  gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer2);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(col2), gl.STATIC_DRAW);

  var colorBuffurs = [colorBuffer0, colorBuffer1, colorBuffer2]
  ...
  for (let i = 0; i < teapots; ++i) {
    ...
    teapotPositions[i] = [Math.cos(angle) * radius, translation[1], Math.sin(angle) * radius];
  }

  var targetPositon = teapotPositions[0];
  ...
  function drawScene(now) {
    ...
    fieldOfViewRadians -= degToRad(0.02) * deltaTime;

    timeElapsed += deltaTime;
    if (timeElapsed >= 2500) {
      targetPositon = teapotPositions[selector];
      fieldOfViewRadians = degToRad(60);
      selector += 1;
      if (selector > teapots - 1) {
        selector = 0;
      }
      timeElapsed = 0;
    }
    ...
    cameraMatrix = m4.translate(cameraMatrix, 0, 20, cameraRadius);
    ...
    var cameraMatrix = m4.lookAt(cameraPosition, targetPositon, up);
    ...
    for (let i = 0; i < teapots; ++i) {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffurs[i]);
      var size = 3;
      var type = gl.FLOAT;
      var normalize = false;
      var stride = 0;
